...
#aG + B, G is basepoint..
def addKeys1(a, B):
    return addKeys(scalarmultBase(a), B)
...
def mul_8key(a):
    return scalarmultKey(a, intToHex(8))
...
# note the reduced version here is for convenience, actually should be non-reduced in general.
def cn_fast_hash(s):
    k = Keccak.Keccak()
...
def hashToPointCN(hexVal):
    u= hexToInt(cn_fast_hash(hexVal)) 
    A = 486662
    ma = -1 * A % q
    ma2 = -1 * A * A % q
    ...
    if (y != 0):
        ...
        if (y != 0) :
            ...
            rx = rx * -1 * ed25519.sqroot(-2 * A * (A + 2) ) % q #fffb1
            negative = False
    else :
        #y was 0..
        rx = (rx * -1 * ed25519.sqroot(2 * A * (A + 2) ) ) % q  #fffb2
